...
app.post("/register", (req, res) => {
    ...
    // hashing the password with more sufisticated algorithm -  bcrypt
    bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
        // once the hash is done:

        // creating a new user in the database
        const newUser = new User({
            email: req.body.email,
            // saving a hash into the database
            password: hash
        });
...
app.post("/login", (req, res) => {
    ...
    // converting password to hash and then comparing to hashes
            ...
            if (!error) {
                ...
                if (userFound) {
                    ...
                    bcrypt.compare(password, userFound.password, function(err, result) {
                        // if the hashes are matching then the result will be true
                        if (result){
                            res.render('secrets');
                        } else {
